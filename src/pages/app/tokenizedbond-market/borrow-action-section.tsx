...
export function BorrowAction() {
	...
	const handleClick = async () => {
		try {
			await placeOrder({
				debtToken: state.token.baseAddress as `0x${string}`,
				collateralToken: state.token.quoteAddress as `0x${string}`,
	...
	// 	// error: debtError,
	// 	// loading: debtLoading,
	// } = useBalance(address!, state.token.baseAddress as `0x${string}`);

	const {
		balance: collateralBalance,
		// error: collateralError,
		// loading: collateralLoading,
	} = useBalance(address!, state.token.quoteAddress as `0x${string}`);
